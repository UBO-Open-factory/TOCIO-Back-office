...
class GenerationController extends Controller {
	public function actionGetdata( $idModule, $nomcarte ) {
		...
		$model_relModuleCapteur = Relmodulecapteur::find()->where( [
				"idModule" => $idModule ] )->orderBy( [
				'ordre' => SORT_ASC ] )->all();

		// on récupère l'id de la carte sélectionné
		$model_carte_id = cartes::find()->where( ["nom" => $nomcarte ] )->one();

		// on récupère toutes les méthodes associé à cette carte
		$model_cartesmethod = relcartesmethod::find()->where( ['id_carte' => $model_carte_id["id"] ] )->all();
		...
		foreach( $$model_relModuleCapteur as $model_relmodulecapteur1 ) {
			...
			foreach( $model_cartesmethod as $model_cartesmethod1 ) {
				...
				if( explode( " ", $model_relmodulecapteur1['nomcapteur'] )[0] === explode( "_", $model_method['nom_method'] )[0] ) {
					...
					foreach( Relcapteurgrandeur::find()->where( [ "idCapteur" => $model_relmodulecapteur1["idCapteur"] ] )->all() as $Grandeur_finded ) {
						$l_TAB_ReadMethod = array();
						$l_TAB_ReadMethod[0] = explode( " ", Grandeur::find()->where( ["id" => $Grandeur_finded["idGrandeur"] ] )->one()["nature"] )[0];
						$l_TAB_ReadMethod[1] = explode( "|CutBalise|", $model_method['method_read'] )[$i];
						$l_TAB_ReadMethod[2] = Grandeur::find()->where( ["id" => $Grandeur_finded["idGrandeur"] ] )->one()["formatCapteur"];
						array_push( $data["grandeur"], $l_TAB_ReadMethod );
						$i++;
					}
			...
			if( $var_method_finded == 0 ) {
				...
				foreach( Relcapteurgrandeur::find()->where( ["idCapteur" => $model_relmodulecapteur1["idCapteur"] ] )->all() as $Grandeur_finded ) {
					$l_TAB_ReadMethod = array();
					$l_TAB_ReadMethod[0] = explode( " ", Grandeur::find()->where( ["id" => $Grandeur_finded["idGrandeur"] ] )->one()["nature"] )[0];
					$l_TAB_ReadMethod[1] = "//No method find for ".explode( " ", $model_relmodulecapteur1['nomcapteur'] )[0];
					$l_TAB_ReadMethod[2] = Grandeur::find()->where( ["id" => $Grandeur_finded["idGrandeur"] ] )->one()["formatCapteur"];
					array_push( $data["grandeur"], $l_TAB_ReadMethod );
					$i++;
				}
