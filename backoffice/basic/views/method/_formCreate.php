...
$listeNomCapteur = array_combine( Capteur::find()->select( [
		'id' ] )->indexBy( 'id' )->column(), Capteur::find()->select( [
...
<div class="method-form">
    ...
    <div class="row">
		<div class="col-sm-3">
            <?= $form->field( $model, 'id_capteur' )->dropDownList( $listeNomCapteur , ['class' => 'form-control id_capteur','id' => 'id_capteur','prompt'=>'Select ...'])?>
            ...
            <?= $form->field( $model, 'nom_method', [
						'options' => [
								'class' => 'invisible' ] ] )->hiddenInput( [
						'value' => '',
						'class' => 'form-control sortie_method',
						'id' => 'sortie_method' ] );
				?>
        </div>
        	...
        	<?php
				...
				foreach( capteur::find()->all() as $taillecapteur ) {
					foreach( relcapteurgrandeur::find()->where( [
							"idCapteur" => $taillecapteur['id'] ] )->all() as $taillegrandeur ) {
						$taillesave++;
					}
        	...
        	<div id = "FullMenu" style ="width: 100%;height: <?php echo $taillemax*100; ?>px;">
	            <?php
					...
					foreach( capteur::find()->all() as $capteurid ) {
						...
						foreach( relcapteurgrandeur::find()->where( [
								"idCapteur" => $capteurid['id'] ] )->all() as $id_relgrandeur ) {
					...
					?>
	            <?php
					echo $form->field( $model, 'method_read', [
							'options' => [
									'class' => 'invisible' ] ] )->hiddenInput( [
							'value' => '',
